...
void onScroll() {
  ...
  if (maxScroll == currentScroll && hasMoreData.value && !isLoadingMore.value) {
    loadMoreData();
  }
}
...
Future loadInitialData() async {
  ...
  try {
    final chatRooms = await _chatRoomsService.getChatRooms(page: page, limit: limit, filter:filter.value );
    final sortedData = chatRooms.data;
    if (sortedData.length < limit) {
      hasMoreData.value = false;
    }
    infoList.addAll(sortedData); // Menggunakan addAll untuk menambahkan data baru
    doctorRoomList.addAll(sortedData.map((info) => info.doctor)); // Menggunakan addAll untuk menambahkan data baru
    page++;
  } catch (e) {
    print('Error: $e');
  }
